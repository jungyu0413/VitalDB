...
class dnn_dataset(torch.utils.data.Dataset):
    def __init__(self, ecg, ppg, target, multi):
        self.multi= multi
        self.ecg, self.ppg = ecg, ppg
        self.target = target
        
    def __getitem__(self, index):
        if self.multi == True: # multi-channel model
            return np.float32( np.vstack (( np.array ( self.ecg[index] ),
                                            np.array ( self.ppg[index] ), ) ) ), np.float32(self.target[index])
        else: # no-channel model (ppg-only model)
            return np.float32( np.array ( self.ppg[index] ) ), np.float32(self.target[index])      
...
class Net(nn.Module):
    def __init__(self, task, multi):
    
        self.task, self.multi = task, multi
...
for phase in [ 'train', 'valid', 'test' ]:
    dataset[phase] = dnn_dataset (ext[phase]['ecg'],
                                    ext[phase]['ppg'],
